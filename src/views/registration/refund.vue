<template>
  <div style="margin-top: 2px">
    ...
    <el-dialog title="发票预览（冲正）" :visible.sync="dialogVisible">
      <div class="invoice">
        <p style="text-align: center"><strong>东软云HIS医院通用发票(冲正)</strong></p>
        ...
        <div v-for="(item,i) in multipleSelection">
          ...
          <p style="text-align: right;white-space: pre">单价 {{item.price*-1}} 数量 {{item.amount}} </p>
        </div>

        <el-divider></el-divider>
        <div style="text-align: right;font-weight: bold">总计：{{totalPrice*-1}} 元  </div>
        <div style="text-align: right; white-space: pre">数量 {{totoalNum}}  项</div>

        <div style="text-align: right;margin-top: 10px;font-size: 20px">退还：{{totalPrice}} 元</div>
